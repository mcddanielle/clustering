/*1.23.19
 *something is wrong with the data structure, so you aren't putting the data in the array,
 and it is segfaulting...
... so pick a more stupid, working data structure...

...eventually get this to qsort() your data...

 *making an n^2 algorithm to find all neighbor distances.
 *using cluster code since it is good at sucking in smtest data
...
main(int argc,char *argv[])
{
  ...
  while(!feof(in)){
    ...
    sprintf(str_time,"%08d",frame); //convert current to a string   
